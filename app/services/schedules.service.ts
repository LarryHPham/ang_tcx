...
export class SchedulesService {
  ...
  getWeatherCarousel(scope, selectedLocation) {
    ...
    //optional week parameters
    return new Observable(observer => {
      observer.next(this.getDummyGraphResult());
      observer.complete();
    });
    // return this.http.get(callURL, { headers: headers })
    //     .map(res => res.json())
  ...
  setupSlideScroll(topScope, data, scope, profile, eventStatus, limit, pageNum, selectedLocation, callback: Function, year?, week?) {
    this.getBoxSchedule(scope, 'league', eventStatus, limit, 1)
      .subscribe(data => {
        ...
        var formattedData = this.transformSlideScroll(scope, data);
  ...
  transformSlideScroll(scope, data) {
    ...
    data.forEach(function(val, index) {
      ...
      let eventTime = val.eventStartTime ? Number(val.eventStartTime) : new Date().getTime();
      let date = moment(eventTime).format('dddd, MMM. D').toUpperCase();
      let time = moment(eventTime).format('h:mm A z');
  ...
  getDummyGraphResult() {
    var output = {
      scopeList: [], blocks: [
        ...
        current_icon: "/app/public/tornado.png",
        ...
        zipcode: 67260,
        background: "/app/public/photo-1473773508845-188df298d2d1.jpeg",
        options: {
          chart: {
            backgroundColor: 'transparent',
            type: 'areaspline',
